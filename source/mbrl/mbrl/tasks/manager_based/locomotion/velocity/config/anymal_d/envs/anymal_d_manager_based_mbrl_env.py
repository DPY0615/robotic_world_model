...
class ANYmalDManagerBasedMBRLEnv(ManagerBasedMBRLEnv): # ANYmalD 的 manager-based MBRL 环境
    ...
    def get_imagination_observation(self, state_history, action_history, observation_noise=True): # 从状态历史和动作历史中获取 imagination observation
        ...
        if observation_noise: # 如果 observation_noise 为 True，则在观测中添加噪声
        ...
        obs = torch.cat([obs_base_lin_vel, obs_base_ang_vel, obs_projected_gravity, self.base_velocity, obs_joint_pos, obs_joint_vel, self.obs_last_action], dim=1) # 将观测的各个部分拼接在一起，形成最终的观测向量
        obs = TensorDict({"policy": obs}, batch_size=[self.num_imagination_envs], device=self.device)
        self.last_obs = obs
        return obs
    

    def _parse_imagination_states(self, imagination_states_denormalized): # 解析 imagination states，将其分为 base_lin_vel、base_ang_vel、projected_gravity、joint_pos、joint_vel 和 joint_torque 等部分
