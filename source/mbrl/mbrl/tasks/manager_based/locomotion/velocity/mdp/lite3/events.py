...
def randomize_rigid_body_inertia( # 随机化刚体惯性张量；环境初始化时调用
    env: ManagerBasedEnv,
    env_ids: torch.Tensor | None,
    asset_cfg: SceneEntityCfg,
    inertia_distribution_params: tuple[float, float],
    operation: Literal["add", "scale", "abs"], # 加随机量；缩放；直接设置为随机值
    distribution: Literal["uniform", "log_uniform", "gaussian"] = "uniform", # 随机分布类型：均匀分布；对数均匀分布；高斯分布
):
...
def randomize_com_positions( # 刚体质心偏移offset随机化；环境初始化时调用
    env: ManagerBasedEnv,
    env_ids: torch.Tensor | None,
    asset_cfg: SceneEntityCfg,
...
def bad_orientation_2( # 终止条件：当机器人姿态过于倾斜时终止；每一步调用
